              ...
              <div className="mt-4 p-3 bg-yellow-50/80 backdrop-blur-sm border border-yellow-300/50 rounded-xl shadow-sm">
                <div className="flex items-center">
                  ...
                  <p className="text-yellow-700 text-xs md:text-sm font-medium">
                    <strong>Mode Lihat Riwayat:</strong> Tidak dapat mengubah data absensi masa lalu.
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="mt-6">
          ...
          {selectedKelas && (
            <p className="mb-4 text-sm text-gray-600 font-medium">
              Menampilkan data untuk: <span className="font-semibold text-blue-600">
                {(kelasList || []).find(k => k.kelas_id == selectedKelas)?.nama_kelas}
              </span>
            </p>
          )}
          ...
          {submitResult && submitResult[0] && (
            <div className={`mb-4 p-3 md:p-4 rounded-xl font-medium text-sm ${
                submitResult[0].message.includes('Berhasil') 
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <button className="bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-700 hover:to-gray-800 text-white font-semibold py-2 px-4 md:py-2.5 md:px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 cursor-pointer hover:-translate-y-0.5 text-sm flex-1 min-w-[120px]"
                    onClick={handleSelectAll}>
                Select All
            </button>
            
            <button className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-semibold py-2 px-4 md:py-2.5 md:px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 cursor-pointer hover:-translate-y-0.5 text-sm flex-1 min-w-[120px]"
                    onClick={markSelectedPresent}
                    disabled={selectedStudents.length === 0}>
                Present
            </button>
            <button className="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white font-semibold py-2 px-4 md:py-2.5 md:px-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 cursor-pointer hover:-translate-y-0.5 text-sm flex-1 min-w-[120px]"
                    onClick={markSelectedAbsent}
                    disabled={selectedStudents.length === 0}>
                Absent
            </button>
          ...
          <div className="block md:hidden space-y-4">
            {filteredMurid.map((murid, index) => (
              <div key={murid.murid_id} className="bg-white/90 backdrop-blur-sm border border-gray-300/50 rounded-2xl p-4 shadow-lg hover:shadow-xl transition-all duration-200">
                {/* Header Card */}
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="font-bold text-gray-800 text-lg">{murid.nama_lengkap}</h3>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                  <div>
                    ...
                    <p className="font-medium text-gray-800">{murid.jenis_kelamin}</p>
                  </div>
                  <div>
                ...
                {/* File Upload */}
                <div className="mb-4">
                  <span className="text-gray-500 text-sm block mb-2">Surat Keterangan</span>
          ...
          <div className="hidden md:block rounded-2xl overflow-hidden border border-gray-300/50 shadow-lg">
            <table className="w-full border-collapse text-center bg-white/80 backdrop-blur-sm">
              <thead className="bg-gradient-to-r from-blue-600 to-purple-600 text-white">
                <tr>
                  {!isViewingHistory && <th className="px-4 py-4 font-semibold"></th>}
                  ...
                  {!isViewingHistory && <th className="px-4 py-4 font-semibold">Aksi</th>}
                </tr>
              </thead>
              <tbody>
                {filteredMurid.map((murid, index) => (
                  <tr key={murid.murid_id} className="border-b border-gray-300/50 hover:bg-gray-50/80 transition-colors duration-150">
                     {!isViewingHistory && ( <td className="px-4 py-4">
                      <input  type="checkbox" 
                              checked={selectedStudents.includes(murid.murid_id)}
                              onChange={() => handleSelectStudent(murid.murid_id)} 
                              disabled={murid.sudah_absen}
                    ...
                    <td className="px-4 py-4 font-medium text-gray-700">{murid.nis}</td>
                    <td className="px-4 py-4 font-medium text-gray-800">{murid.nama_lengkap}</td>
                    <td className="px-4 py-4 text-gray-600">{murid.jenis_kelamin}</td>
          ...
          <div className="mt-6 p-4 bg-gradient-to-r from-blue-50 to-purple-50/80 backdrop-blur-sm border border-blue-200/50 rounded-xl shadow-sm">
            <h4 className="font-semibold text-gray-800 mb-3 text-sm md:text-base">Summary Absensi Hari Ini:</h4>
